...
function OfferCard({
  ...
  // Staff can accept/reject client offers (sender_role === "client")
  const is_client_offer = msg.sender_role === "client";
  const can_act = msg.offer_status === "pending" && is_staff && is_client_offer;

  const handle = async (action: "accept" | "reject") => {
    try {
      set_loading(true);
      if (action === "accept") {
        await otcService.admin_accept_offer(order_uid, msg.id);
        toast.success(t("otc.messages.offerAccepted"));
      } else {
        await otcService.admin_reject_offer(order_uid, msg.id);
  ...
  const is_mine = !is_client_offer; // operator's messages are "mine"

  return (
    <div className={`max-w-sm ${is_mine ? "ml-auto" : "mr-auto"}`}>
      <div className="border rounded-xl p-4 bg-white shadow-sm">
        <div className="text-xs font-semibold text-gray-500 mb-2 uppercase tracking-wide">
          {t("otc.chat.sendOffer")} · {msg.sender_role}
        </div>
        ...
        {can_act && (
          <div className="flex gap-2 mt-3">
            <Button size="sm" className="flex-1" onClick={() => handle("accept")} disabled={loading}>
              {t("otc.chat.accept")}
            </Button>
            <Button size="sm" variant="outline" className="flex-1" onClick={() => handle("reject")} disabled={loading}>
              {t("otc.chat.reject")}
            </Button>
          </div>
        )}
        ...
        {msg.offer_status && msg.offer_status !== "pending" && (
          ...
          >
            {msg.offer_status === "accepted" ? t("otc.chat.accepted") : t("otc.chat.rejected")}
          </div>
        )}
      </div>
...
export function AdminOTCOrderDetail() {
  const { uid } = useParams<{ uid: string }>();
  const navigate = useNavigate();
  const { t } = useTranslation();
  const user = useAuthStore((state) => state.user);
  const access_token = useAuthStore((state) => state.access_token);
  const is_staff = ["operator", "admin", "super_admin"].includes(user?.role ?? "");
  ...
  const run_action = async (fn: () => Promise<void>, success_key: string) => {
    try {
  ...
  const send_offer = async () => {
    ...
    try {
      ...
      await otcService.admin_send_offer(uid, offer_form);
  ...
  const cancel_order = async () => {
    if (!uid) return;
    await run_action(
      () => otcService.admin_cancel_order(uid, cancel_reason),
      "otc.messages.orderCancelled"
    );
  ...
  const is_terminal = TERMINAL_STATUSES.includes(order.status);
    ...
    <div className="space-y-6 max-w-3xl mx-auto">
      ...
      <div className="bg-white rounded-xl shadow-sm p-6">
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4 text-sm">
          <div>
            <div className="text-gray-500">{t("admin.users.id")}</div>
            <div className="font-semibold mt-0.5">User #{order.user_id}</div>
          </div>
          {order.operator_id && (
            <div>
          ...
          {order.agreed_from_amount && (
            <div>
              <div className="text-gray-500">{t("otc.fields.agreedFromAmount")}</div>
              <div className="font-semibold mt-0.5">
                {order.agreed_from_amount.toLocaleString()} {from_code}
              </div>
            </div>
          )}
          {order.to_amount && (
        ...
        {/* Payment deadline */}
        {(order.status === "awaiting_payment" || order.status === "expired") && order.payment_deadline && (
          ...
          <div className="mt-4 pt-4 border-t flex flex-wrap gap-2">
            {order.status === "awaiting_review" && (
              <Button
                size="sm"
                onClick={() =>
                  run_action(() => otcService.admin_take_order(order.uid), "otc.messages.orderTaken")
                }
            ...
            {order.status === "negotiating" && (
              <Button
                ...
                onClick={() =>
                  run_action(
                    () => otcService.admin_accept_as_proposed(order.uid),
                    "otc.messages.acceptedAsProposed"
                  )
                }
                disabled={action_loading}
              >
                <CheckCircle className="h-4 w-4 mr-1" />
                {t("otc.actions.acceptAsProposed")}
              </Button>
            )}
            ...
            {(order.status === "awaiting_payment" || order.status === "expired") && (
              <Button
                ...
                onClick={() =>
                  run_action(
                    () => otcService.admin_confirm_payment(order.uid),
                    "otc.messages.paymentConfirmed"
                  )
                }
                disabled={action_loading}
              >
                <Banknote className="h-4 w-4 mr-1" />
                {t("otc.actions.confirmPayment")}
              </Button>
            )}
            ...
            {order.status === "payment_received" && (
              <Button
                ...
                onClick={() =>
                  run_action(
                    () => otcService.admin_complete_order(order.uid),
                    "otc.messages.orderCompleted"
                  )
                }
                disabled={action_loading}
              >
                <CheckCircle className="h-4 w-4 mr-1" />
                {t("otc.actions.complete")}
              </Button>
            )}
      ...
      <div className="bg-white rounded-xl shadow-sm overflow-hidden">
        ...
        <div className="p-6 space-y-4 min-h-48 max-h-[480px] overflow-y-auto">
          ...
          {(order.messages ?? []).map((msg) => {
            const is_staff_msg = msg.sender_role !== "client";
            ...
            return (
              <div key={msg.id} className={`flex flex-col ${is_staff_msg ? "items-end" : "items-start"}`}>
                <div
                  className={`max-w-xs px-4 py-2 rounded-2xl text-sm ${
                    is_staff_msg ? "bg-blue-600 text-white" : "bg-gray-100 text-gray-900"
                  }`}
                >
                  {msg.content}
                </div>
          ...
          <div className="space-y-4 mt-2">
            ...
            <div className="flex gap-3">
              ...
              <Button
                ...
                disabled={action_loading}
              >
                {action_loading ? t("common.saving") : t("otc.actions.cancel")}
              </Button>
            </div>
          </div>
        </DialogContent>
