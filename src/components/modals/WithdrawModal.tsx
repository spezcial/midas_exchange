...
import { ActionVerifyModal } from "./ActionVerifyModal";
...
export function WithdrawModal({
  ...
  // Pending form values saved while the verify modal is open
  const [pending_values, set_pending_values] = useState<{ amount: number; address: string } | null>(null);
  ...
  const on_submit = (values: WithdrawFormValues) => {
    set_pending_values({ amount: values.amount, address: values.address || "" });
    set_show_verify(true);
  };

  const on_verified = async (action_token: string) => {
    if (!pending_values) return;
    set_show_verify(false);
    set_is_submitting(true);
    try {
      await walletService.withdraw({
        currency: wallet.currency.code as any,
        amount: pending_values.amount,
        address: pending_values.address,
        action_token,
      });
      toast.success(t("messages.withdrawRequestCreated", { amount: pending_values.amount, currency: wallet.currency.code }));
      form.reset();
      set_pending_values(null);
      on_success();
    } catch (err: unknown) {
      const error_message = (err as { response?: { data?: { error?: string } } })?.response?.data?.error || t("messages.withdrawFailed");
  ...
  return (
    <>
      <Dialog open={is_open && !show_verify} onOpenChange={(open) => { if (!open) on_close(); }}>
      ...
      <ActionVerifyModal
        ...
        on_close={() => { set_show_verify(false); set_pending_values(null); }}
      />
    </>
  );
}
